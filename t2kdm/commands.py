...
class Command(object):
    ...
    def __init__(self, name, function, description="", **kwargs):
        ...
        if "%s-cli" % (dm._branding) in sys.argv[0]:
            self.parser = argparse.ArgumentParser(
                prog=name, description=description, **kwargs
            )
        else:
        ...
        self.parser.add_argument(
            "--version",
            action="version",
            version="{brand} {version}".format(
                brand=dm._branding, version=dm.__version__
            ),
        )
    ...
    def _condition_argument(name, value, localdir=None, remotedir=None):
        ...
        if (
            remotedir is not None
            and "remotepath" in name
    ...
    def run(self, parsed_args, **kwargs):
        ...
        for arg in self.positional_arguments:
            value = self._condition_argument(
                arg, getattr(parsed_args, arg), localdir=localdir, remotedir=remotedir
            )
...
ls.add_argument(
    "-l", "--long", action="store_true", help="longer, more detailed output"
)
ls.add_argument(
    "-d",
    "--directory",
...
replicas = Command(
    "replicas", interactive.replicas, "List replicas of a remote logical path."
)
...
check.add_argument(
    "remotepath", type=str, help="the remote logical path, e.g. '/nd280'"
)
check.add_argument(
    "-r",
    "--recursive",
...
replicate = Command(
    "replicate", interactive.replicate, "Replicate file to a storage element."
)
replicate.add_argument(
    "remotepath", type=str, help="the remote logical path, e.g. '/nd280/file.txt'"
)
replicate.add_argument(
    "destination",
    type=str,
...
get.add_argument(
    "localpath", type=str, nargs="?", default="./", help="the local path, e.g. `./`"
)
get.add_argument(
    "-f", "--force", action="store_true", help="overwrite local files if necessary"
)
get.add_argument(
    "-r",
    "--recursive",
...
put.add_argument("localpath", type=str, help="the file to be uploaded")
put.add_argument(
    "remotepath", type=str, help="the remote logical path, e.g. '/nd280/file.txt'"
)
put.add_argument(
...
SEs = Command(
    "SEs",
    interactive.print_storage_elements,
    "Print all available storage elements on screen.",
)
...
remove = Command(
    "remove",
    interactive.remove,
    "Remove file replica from a storage element, if it is not the last one.",
)
remove.add_argument(
    "remotepath", type=str, help="the remote logical path, e.g. '/nd280/file.txt'"
)
remove.add_argument(
    "destination",
    type=str,
    help="the destination storage element by name, e.g. 'UKI-SOUTHGRID-RALPP-disk', or by host, e.g. 't2ksrm.nd280.org', or special `any` for removing files w/o any registered replicas",
)
remove.add_argument(
    "-f",
    "--final",
...
rmdir = Command(
    "rmdir", interactive.rmdir, "Remove empty directory from the catalogue."
)
rmdir.add_argument(
    "remotepath", type=str, help="the remote logical path, e.g. '/nd280/dir/'"
)
...
html_index = Command(
    "html_index",
    interactive.html_index,
    "Generate HTML index of a catalogue directory.",
)
html_index.add_argument(
    "remotepath", type=str, help="the remote logical path, e.g. '/nd280/'"
)
html_index.add_argument(
    "localpath",
...
rename.add_argument(
    "remotepath", type=str, help="the old remote logical path, e.g. '/nd280/file.txt'"
)
rename.add_argument(
    "regex_from", type=str, help="the regular expression to be replaced, e.g. 't(.)t'"
)
rename.add_argument(
    "regex_to", type=str, help="the regular expression to be put inplace, e.g. 'T\\1T'"
)
rename.add_argument(
    "-v", "--verbose", action="store_true", help="print status messages to the screen"
)
